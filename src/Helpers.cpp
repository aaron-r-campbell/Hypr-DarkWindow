...
static std::string editShader(std::string source,
                              const std::string &windowShader) {
  ...
  if (out != std::string::npos) {
    ...
    outVar =
        std::string(&source[out + sizeof("layout(location = 0) out vec4 ") - 1],
                    &source[endOut]);
    ...
    if (!source.contains("gl_FragColor")) {
      Log::logger->log(
          Log::ERR, "Failed to edit GLSL Code, no gl_FragColor:\n{}", source);
  ...
  if (!source.contains("void main(")) {
    Log::logger->log(Log::ERR, "Failed to edit GLSL Code, no main function: {}",
                     source);
    throw efmt("Frag source does not contain an occurence of 'void main('");
  }

  Hyprutils::String::replaceInString(source, "void main(",
                                     "void main_unshaded(");
...
ShaderDefinition::ShaderDefinition(std::string id, std::string from,
                                   std::string path, const std::string &args,
                                   bool transparency)
...
void ShaderHolder::PrimeUniforms(const Uniforms &args) {
  ...
  for (auto &[_, s] : Shaders) {
    for (auto &[name, _] : args) {
      if (s.UniformLocations.contains(name))
        continue;

      GLint loc = glGetUniformLocation(s.Shader->program(), name.c_str());
      if (loc == -1)
        throw efmt("Shader failed to find the uniform: {}", name);
...
ShaderHolder::ShaderHolder(const std::string &source) {
  ...
  for (auto &[id, fragFile] : {
           std::pair{SH_FRAG_EXT, "ext.frag"},
       }) {
    // skip CM s if hyprland skipped them too
    if (!g_pHyprOpenGL->m_shaders->frag[id])
      continue;
